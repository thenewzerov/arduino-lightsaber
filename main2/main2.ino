...
uint8_t  TRACK_SABER_HUM  = 2;
uint8_t  TRACK_SABER_SWING_A = 3;
uint8_t  TRACK_SABER_SWING_B = 4;
uint8_t  TRACK_SABER_CLASH_A = 5;
uint8_t  TRACK_SABER_CLASH_B = 6;
uint8_t  TRACK_SABER_OFF = 7;
uint8_t  trackQueue = 6;
...
// or called locally when not in callback mode.
{
  ...
  case MD_YX5300::STS_ERR_FILE:   Serial.print(F("STS_ERR_FILE")); audioPlaying = false; Serial.println("audioPlaying = false"); break;
  case MD_YX5300::STS_ACK_OK:     Serial.print(F("STS_ACK_OK"));     break;
  case MD_YX5300::STS_FILE_END:   Serial.print(F("STS_FILE_END")); audioPlaying = false; Serial.println("audioPlaying = false");  break;
...
void checkAudio(boolean override){
  ...
  if(!audioPlaying || override){
    Serial.println("No song playing");
    //If we don't have anything else to play, play the hum
    if(trackQueue == 0){
      Serial.println("Playing HUM");
      mp3.playSpecific(FOLDER, TRACK_SABER_HUM);
      mp3.playSpecific(FOLDER, TRACK_SABER_HUM);
      Serial.println("audioPlaying = true");
      audioPlaying = true;
    }else{
      Serial.println("Playing Track" + trackQueue);
